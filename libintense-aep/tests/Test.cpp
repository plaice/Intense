...
(int argc, char *argv[])
{
  ...
  while ((c = getopt(argc, argv, "ht:mn:")) != -1) {
    switch (c) {
    case 'h':
      // Help:
      usage();
    ...
    case 't':
      ...
      if (nThreads < 1) {
        ...
        usage();
        exit(1);
      } else if ((nThreads > 1)&&(!permitMultiThreadedTest())) {
        cerr << "Only one (main) thread permitted (-t<nThreads> "
             << "not supported)\n";
        exit(1);
      }
      break;
    ...
    case 'n':
      nTimes = atoi(optarg);
      if (nTimes < 1) {
        cerr << "\nError: negative number of test iterations specified!\n";
        usage();
        exit(1);
      }
      break;
...
(void* arg)
{
  ...
  } else if (test->nTimes > 0) {
    for (int i = 0; i < test->nTimes; i++) {
      test->test();
    }
  } else {
    test->test();
